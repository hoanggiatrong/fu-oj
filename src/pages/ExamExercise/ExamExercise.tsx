...
const ExamExercise = observer(() => {
    const { examId, exerciseId, groupExamId } = useParams();
    ...
    useEffect(() => {
        ...
        http.get(`/exam-rankings?userId=${userId}&groupExamId=${groupExamId}`)
            .then((res) => {
                if (Array.isArray(res.data) && res.data.length > 0) {
                    const ranking = res.data[0];
                    console.log('ranking', ranking);
                    // if (ranking.groupExamId || ranking.groupExam?.groupExamId || ranking.groupExam?.id) {
        ...
        http.post('/exams/submissions', payload)
            .then((res) => {
                ...
                if (res.status === 201 && examId) {
                    console.debug(`/${routesConfig.exam}`.replace(':groupExamId', groupExamId));
                    navigate(`/${routesConfig.exam}`.replace(':groupExamId', groupExamId));
                }
            })
            .catch((error) => {
